...
echo "deb file:/usr/local/src/redcheck-astra-repo/ 1.7_x86-64 non-free dotnet" | sudo tee /etc/apt/sources.list.d/redcheck.list
sudo apt -y update
sudo apt -y install redcheck-dotnet-runtime redcheck-aspnetcore-runtime redcheck-api redcheck-client redcheck-scan-service redcheck-sync-service redcheck-cleanup-service
